...
describe("prompt artifact hints", () => {
  ...
  it("pushes planning and execution toward a shared system spine instead of isolated surfaces", () => {
    const planningPrompt = buildPhasePrompt(
      {
        ...makeContext(),
        phase: "planning",
        previousJudge: {
          ...
          totalScore: 82,
          hiddenCheckScore: 96,
          productQualityScore: 74,
          byCategory: { repo: 100, product_quality: 74 },
          passedRequired: true,
          confidence: 0.8,
          failedChecks: [],
          passedChecks: [],
          ...
          recommendations: [],
          productReview: {
            summary: "Good breadth, weak center.",
            overallScore: 74,
            axes: {
              product_depth: 72,
              architecture_coherence: 76,
            },
            findings: [
    ...
    expect(planningPrompt).toContain("shared system spine");
    expect(planningPrompt).toContain("multiple adjacent feature islands");

    const executionPrompt = buildPhasePrompt(makeContext(), []);
    expect(executionPrompt).toContain("multiple surfaces");
    expect(executionPrompt).toContain("shared model, engine, service, or state layer");
  });
});

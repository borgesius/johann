...
function buildVisibleBacklog(
  ...
  if (state.cycles.length === 0 && state.stopRules.continueAfterSuccess) {
    const synthetic = [
      {
        id: "synthetic-depth-loop",
        bucket: "quality_improvement" as const,
        title: "Prove one real core loop with state changes and consequences",
        rationale:
          "Use the first serious cycle to build more than a shell. After the minimum scaffold exists, implement one end-to-end mechanic where user input changes domain state and produces visible consequences in the product.",
        source: "synthetic-depth",
        severity: 5,
      },
      {
        id: "synthetic-domain-system",
        bucket: "quality_improvement" as const,
        title: "Implement a real domain/system module, not just UI shell",
        rationale:
          "Prefer one meaningful game/domain/service module over extra docs, test harnesses, or static placeholder UI. The repo should start proving its actual product logic early.",
        source: "synthetic-depth",
        severity: 4,
      },
      {
        id: "synthetic-system-spine",
        bucket: "quality_improvement" as const,
        title: "Create a shared system spine that multiple surfaces depend on",
        rationale:
          "Do not let a long-horizon run turn into several isolated panels or tools. Early on, identify and start implementing the underlying model, engine, service, or state layer that will tie multiple product surfaces together.",
        source: "synthetic-depth",
        severity: 4,
      },
    ];
...
async function runCycle(
  ...
  const previousFinalJudge = state.finalJudge;
  cycle.judge = judge;
  state.finalJudge = judge;

...
function buildPhaseInstructions(phase: Exclude<PhaseId, "judging">): string {
  switch (phase) {
    ...
    case "pm_reprioritization":
      return "Act as a PM/TL again. Take the latest judge feedback and convert it into the next priority queue while maintaining long-term architecture directives and updating the hierarchical work breakdown. If the remaining work is still too big, split it again rather than pretending it fits in one more pass. Explicitly promote the best exploration ideas into delivery when the product needs depth more than another round of checklist cleanup. Maintain a durable initiative queue so the run can pursue medium-horizon product arcs instead of only reacting to the latest failed checks. Balance desired spec coverage with technical quality and overall product feel when choosing what happens next. If the product is already rich but live smoke is failing, prioritize runtime closure ahead of further conceptual or content expansion. If the product is becoming a cluster of adjacent features, make the next cycle about a stronger shared system spine instead of more breadth. Be precise about what execution should do next, but do not perform that implementation from this phase.";
  }
}

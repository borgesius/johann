...
function isSimulationLikeBrief(benchmark: {
  title: string;
  summary: string;
  publicBrief: string;
}): boolean {
  const text = `${benchmark.title}\n${benchmark.summary}\n${benchmark.publicBrief}`.toLowerCase();
  return /(simulation|management game|gameplay|player loop|vertical slice|market dynamics|staff|engine grid)/.test(text);
}
...
function renderProductReview(context: RunnerPhaseContext): string {
  ...
  return [
    ...
    review.evaluationPlan && review.evaluationPlan.length > 0
      ? `- evaluation plan: ${review.evaluationPlan.slice(0, 4).join("; ")}`
      : undefined,
    review.longHorizonPlan && review.longHorizonPlan.length > 0
      ? `- long-horizon plan: ${review.longHorizonPlan.slice(0, 4).join("; ")}`
      : undefined,
    review.improvementHypotheses && review.improvementHypotheses.length > 0
      ? `- improvement hypotheses: ${review.improvementHypotheses.slice(0, 4).join("; ")}`
      : undefined,
...
function renderCompletionPressure(context: RunnerPhaseContext): string {
  ...
  if (
    ...
    && (judge.validationResults ?? []).some(
      (result) => !result.passed && (result.id === "browser-smoke" || /browser smoke/i.test(result.label)),
    )
  ) {
    notes.push(
      "The app is already rich enough that live runtime closure now matters more than adding more content. Stop expanding the experience until browser smoke passes cleanly.",
    );
  }
  if (
    ...
    notes.push(
      "The repo is drifting toward multiple adjacent feature islands. Use the next cycle to create or deepen a shared system spine that several surfaces depend on, rather than adding another standalone panel or tool.",
    );
  }
  if (
    (judge.metaReview?.findings ?? judge.productReview?.findings ?? []).some((finding) =>
...
function renderPhaseConstraints(phase: Exclude<PhaseId, "judging">): string {
  switch (phase) {
    case "pm_intake":
      return `- mode: read_only\n- allowed actions: list_files, read_file, finish\n- guidance: inspect briefly, set backlog and architecture direction, estimate the work in LLM-loop terms, split anything large into smaller leaves, and if this is a multi-hour program, create 3 to 7 major slices. Major slices may be \`delivery\` or \`exploration\` tracks. Open only the first concrete slice into current-cycle leaves, but keep the broader exploration visible. If you already know the first files or scaffold shape, express that in the handoff rather than trying to create files now. Finish with the best structured handoff even if some details remain uncertain`;
    case "planning":
      return `- mode: read_only\n- allowed actions: list_files, read_file, finish\n- guidance: use the available branch templates if branching is worth the budget; if no template fits, you may propose a runtime branch hinge and 2 to 3 concrete candidates. Prefer leaves that are tiny, small, or medium for the current cycle, keep splitting any large or oversized items, and keep the next cycle focused inside one major program slice unless a tiny cross-cutting task is strictly required. Use \`exploration\` tracks when the product needs freeform design or discovery work that should remain open-ended for later passes. If implementation ideas are obvious, capture them as current work, planned files, or recommendations instead of editing the repo. Finish with a practical plan instead of waiting for perfect certainty`;
    case "execution":
      return `- mode: repo_editing\n- allowed actions: list_files, read_file, write_file, replace_in_file, run_command, run_app_smoke, finish\n- guidance: do the minimum coherent implementation that clears the active execution leaves before polishing. If a web surface exists, prefer a short browser smoke over guesswork. Once the app is substantively built, repeated smoke failures should trigger runtime-closure work rather than another round of content expansion.`;
    case "review":
      return `- mode: read_only\n- allowed actions: list_files, read_file, run_command, run_app_smoke, finish\n- guidance: do not edit the repo in review; inspect what changed, call out spec drift, and recommend the next fixes. If the repo exposes a UI or server shell, use a browser smoke pass when it materially improves confidence. Prefer at least one meaningful validation command when the repo appears runnable so review can judge the product holistically instead of by files alone. For Electron apps, explicitly inspect package.json main, build output layout, and preload/renderer wiring instead of assuming a passing TypeScript build means the desktop app will launch correctly.`;
    case "pm_reprioritization":
      return `- mode: read_only\n- allowed actions: list_files, read_file, finish\n- guidance: convert judge and review output into the next backlog, preserve architecture directives, split any remaining oversized work instead of pretending it fits in one more pass, and explicitly decide which exploration opportunities should now be promoted into delivery work. Do not implement fixes here; describe the next implementation slice precisely enough that execution can act on it.`;
  }
}
...
function buildPhaseInstructions(phase: Exclude<PhaseId, "judging">): string {
  switch (phase) {
    case "pm_intake":
      return "Act as a PM/TL. Turn the brief and current repo state into a practical backlog, set early architecture directives, and produce a hierarchical work breakdown. Size the work in benchmark-loop terms, not human-engineer project terms: tiny means one focused execution pass, small means one cycle, medium means a few coordinated passes, large means it should be split before execution, and oversized means it should become multiple benchmark stages. For big briefs, first create a top-level program map with both delivery slices and exploration tracks, then open only the first concrete slice into current-cycle leaves. Exploration tracks are real work: they should generate hypotheses, design options, and future delivery slices instead of sitting idle. Also surface 2 to 5 durable initiatives that represent the strongest medium-horizon bets for making the product excellent. Identify the likely shared system spine early: name the state/model/engine/service layer that multiple product surfaces should eventually depend on, so the program does not dissolve into isolated subtools. If the product review already includes an evaluation plan, treat at least one proof or comparison step as real near-term work once urgent breakages are covered, especially when the run claims to be improving itself or its own workflow. If the initial scaffold feels obvious, describe it in current work and recommendations instead of trying to build it now.";
    case "planning":
      return "Choose an implementation plan, refine the architecture directives, call out the test strategy, and decide whether branching is justified at the available hinge. If no existing hinge fits but comparing two approaches would materially improve decision quality, propose a runtime branch hinge with 2 to 3 concrete candidates. Maintain a hierarchical work breakdown and keep splitting large or oversized items until the next active leaves look executable in this run. Treat the top-level breakdown as a program map, allow exploration tracks where discovery is needed, and keep the current cycle focused inside one major slice when possible. If the initiative or opportunity queue already contains strong exploration ideas, decide whether any of them should be promoted into delivery in this cycle. Plan toward a fuller final product, not a thin pass on the visible spec. Explicitly decide what the shared system spine is for this cycle: which underlying model, engine, service, or state layer should multiple surfaces flow through by the end of the run. When the product review includes an evaluation plan, convert at least one of those proof or comparison steps into explicit active leaves, tests, or follow-up validations once the main blocking breakages are covered. When the first implementation steps are obvious, express them as active leaves, planned files, and validation strategy rather than editing the repo from planning.";
    case "execution":
      return "Do the real work in the repo. Edit files, run commands when useful, and leave the tree in a better state than you found it. Stay inside the active execution leaves unless a small enabling change is required to unblock them. For service repos, prefer a test-friendly shape: keep implementation under src when the brief calls for modularity, export the app or server from the entrypoint, and guard automatic listening so tests can import the server without port conflicts. If you add or change tests, run them and repair obvious failures before finishing; passing tests are worth more than extra polish. When a repo already has a test runner or framework, prefer repairing the existing setup instead of swapping to a different harness. Once the repo has enough surface to be runnable, do not leave execution without at least one passing validation path such as a test, build, typecheck, or smoke path. If the product review already includes an evaluation plan, do not spend the entire cycle on implementation alone: after landing the main causal change, use some remaining budget on at least one proof step such as a comparison run, focused validation, experiment record, or before/after check that could show whether the improvement was real. For greenfield product briefs, do not spend the whole cycle on shell/config/docs work: after the minimum coherent scaffold is stable, use the remaining execution budget on one real product slice where user input changes state and produces visible consequences. If the active execution leaves include both foundation work and deeper product/system work, do not stop after only the foundation slice; land at least one real domain or gameplay module in code this cycle. When the repo already has multiple surfaces, prefer strengthening the shared model, engine, service, or state layer that ties them together over adding another isolated surface. Never gut a core product surface into a trivial placeholder just to rescue build, lint, or smoke status; if a key page or component is broken, repair it without collapsing its actual content and intent. When the checklist is ahead of the product, spend the cycle making the result feel more whole, coherent, and technically sound rather than merely more complete on paper. If browser smoke has already failed on a substantively built web app, stop broadening the product and spend the remaining execution budget on runtime closure until the app actually boots and renders cleanly.";
    case "review":
      return "Inspect the repo critically as if you were deciding whether this is becoming a strong final product. Look for spec drift, code quality issues, missing tests, operational gaps, shallow or disconnected product surfaces, and places where execution drifted outside the active work scope. Prefer a short validation or smoke command over setup churn. For Electron apps, explicitly verify runtime coherence: package.json main should point at the real runtime entry, build output should line up with what Electron launches, and preload/renderer paths should make sense together. For web apps that already look materially built, call out a rich-but-not-live state explicitly when browser smoke still fails, and recommend runtime closure before more content work. Do not spend review turns on dependency installation or long-lived foreground servers unless a previous validation clearly failed because dependencies were missing or a smoke action explicitly needs a start command.";
    case "pm_reprioritization":
      return "Act as a PM/TL again. Take the latest judge feedback and convert it into the next priority queue while maintaining long-term architecture directives and updating the hierarchical work breakdown. If the remaining work is still too big, split it again rather than pretending it fits in one more pass. Explicitly promote the best exploration ideas into delivery when the product needs depth more than another round of checklist cleanup. Maintain a durable initiative queue so the run can pursue medium-horizon product arcs instead of only reacting to the latest failed checks. Balance desired spec coverage with technical quality and overall product feel when choosing what happens next. If the latest meta-review includes an evaluation plan, keep at least one concrete proof or comparison step alive in the next backlog so the run can tell whether it is actually getting better. If the product is already rich but live smoke is failing, prioritize runtime closure ahead of further conceptual or content expansion. If the product is becoming a cluster of adjacent features, make the next cycle about a stronger shared system spine instead of more breadth. Be precise about what execution should do next, but do not perform that implementation from this phase.";
  }
}
...
export function buildPhasePromptWithPreviews(
  ...
  const simulationBriefNote = isSimulationLikeBrief(context.benchmark)
    ? "Simulation/game brief note: do not let cycle 1 collapse into pure shell work. In PM/TL and planning, current-cycle leaves should include both a foundation slice and at least one tiny but real simulation/gameplay/domain slice unless the repo is still fully empty. In execution, once the minimum app foundation is stable, land a mechanic where player input changes state and produces visible downstream consequences."
    : undefined;
...
${simulationBriefNote ? `Product-shape note:\n${simulationBriefNote}\n` : ""}
...
- In read-only phases, never emit write_file, replace_in_file, run_command, or run_app_smoke. Inspect briefly, then finish with a structured handoff.
- In PM/TL, planning, and reprioritization phases, do not jump ahead into implementation even if the repo is empty or the first files feel obvious. Name the intended scaffold in the plan and work breakdown instead of trying to create it.
